...
import com.common.utils.CsvUtil;
...
public class BaseController extends BaseCommon {
     ...
     * 将数据以JSON格式写到客户端
     */
    protected void writeToJson(HttpServletResponse response, Object obj) {
        try {
     ...
     * 统一下载CSV文件
     */
    protected <T> void downCsv(HttpServletResponse response, List<String> head, List<T> dataList, Class<T> clazz) {
        OutputStream out = null;
        try {
            response.setCharacterEncoding(CsvUtil.ENCODE);
            out = response.getOutputStream();
            CsvUtil.createBeanToCSV(out, head, dataList, clazz);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    protected Map<String, Object> basePageParam(HttpServletRequest request, HttpServletResponse response) {
        ...
        Map<String, Object> params = RequestUtil.getSimpleQueryParams(request);
